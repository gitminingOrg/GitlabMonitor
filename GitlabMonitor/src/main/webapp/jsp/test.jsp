...
<div id="chart"></div>
    ...
    <script type="text/javascript">
      $.post("/GitlabMonitor/dayhour",function(data){
			datasets = eval("("+data+")");
	      ...
	      var timeLabels = svg.selectAll(".timeLabel")
	          ...
	          var colorScale = d3.scale.quantile()
	              .domain([0, buckets - 1, d3.max(datasets.value, function (d) { return d; })])
	              .range(colors);

	          var cards = svg.selectAll(".hour")
	              .data(datasets.day, function(d,i) {return datasets.day[i]+':'+datasets.hour[i];});

	          cards.append("title");

	          cards.enter().append("rect")
	              .attr("x", function(d,i) { return (datasets.hour[i] - 1) * gridSize; })
	              .attr("y", function(d,i) { return (datasets.day[i] - 1) * gridSize; })
	              .attr("rx", 4)
	              .attr("ry", 4)
	          ...
	          cards.transition().duration(1000)
	              .style("fill", function(d,i) { return colorScale(datasets.value[i]); });

	          cards.select("title").text(function(d,i) { return datasets.value[i]; });
